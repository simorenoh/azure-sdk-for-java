...
try {
    ...
    if (!$resourceGroupRoleAssigned) {
        ...
        if ($principalOwnerAssignment.RoleDefinitionName -eq 'Owner') {
            ...
            Write-Warning "The 'Owner' role for '$ResourceGroupName' could not be assigned. You may need to manually grant the role to the Test Application '$TestApplicationId' if it does not have subscription-level permissions."
        }
    }
...
privisioning the test resources. If not, specified New-TestResources.ps1 uses the
